...
#   let(:reservation) { create(:reservation, user: user, motorcycle: motorcycle) }

#   describe 'GET #index' do
#     it 'returns all reservations for the current user' do
#       sign_in(user)
#       get :index
#       expect(response).to have_http_status(:success)
#       expect(response.body).to include(reservation.motorcycle.name)
#     end
#   end

#   describe 'GET #show' do
#     it 'returns a reservation if found' do
#       sign_in(user)
#       get :show, params: { id: reservation.id }
#       expect(response).to have_http_status(:success)
#       expect(response.body['id']).to eq(reservation.id)
#     end

#     it 'returns not_found if the reservation is not found' do
#       sign_in(user)
...
#     let(:valid_params) { { start_date: Date.today, end_date: Date.tomorrow, city: 'Test City', user_id: user.id, motorcycle_id: motorcycle.id } }
#     let(:invalid_params) { { start_date: '', end_date: '', cityI apologize for the incomplete response earlier. Here's the complete code for the `MotorcyclesController` and `ReservationsController` tests:
